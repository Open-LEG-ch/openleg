...
class TestMunicipalityOutreachTemplate:
    def test_template_exists(self):
        path = os.path.join(
            PROJECT_ROOT, "templates", "emails", "municipality_outreach.html"
        )
...
class TestGetMunicipalityDemandContext:
    def test_with_signal_returns_populated_context(self):
        ...
        with patch(
            "insights_engine.compute_municipality_demand_signal",
            return_value=_HIGH_SIGNAL,
        ):
    ...
    def test_context_keys_always_present(self):
        ...
        for signal in (_HIGH_SIGNAL, _EMPTY_SIGNAL):
            ...
            assert required_keys.issubset(ctx.keys()), (
                f"Missing keys: {required_keys - ctx.keys()}"
            )
