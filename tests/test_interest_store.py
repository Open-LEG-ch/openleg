...
def test_retention_horizons_are_named_and_used():
    ...
    assert "NOT EXISTS (SELECT 1 FROM consents" in sql
    assert "NOT EXISTS (SELECT 1 FROM referrals" in sql
    assert "NOT EXISTS (SELECT 1 FROM community_members" in sql
