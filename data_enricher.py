...
def get_pv_potential_from_coords(lat, lon):
    ...
    try:
        ...
        print(
            f"  [GEO OK] -> Produktion: {potential_kwh_pa} kWh/a ({potential_kwp:.1f} kWp)"
        )
...
def estimate_consumption_kwh(
    building_type, num_apartments, energy_surface_m2, plz_stats
):
...
def estimate_ev_kwh(building_type, plz_stats):
    ...
    if (
        building_type == "EFH"
    ):  # EFH hat höhere Wahrscheinlichkeit für private Ladestation
...
def get_energy_profile_for_address(address_string):
    ...
    base_consumption_kwh_pa = estimate_consumption_kwh(
        gwr_data[0], gwr_data[1], gwr_data[2], plz_stats
    )
...
if __name__ == "__main__":
    ...
    print("Testaufruf der ECHTEN API mit 'Bundesplatz, 3011 Bern':")
    estimates, _ = get_energy_profile_for_address("Bundesplatz, 3011 Bern")
