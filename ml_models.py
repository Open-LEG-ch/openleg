...
def calculate_distance(lat1, lon1, lat2, lon2):
    ...
    a = sin(delta_phi / 2) * sin(delta_phi / 2) + cos(phi1) * cos(phi2) * sin(
        delta_lambda / 2
    ) * sin(delta_lambda / 2)
...
def generate_mock_profiles(
    annual_consumption_kwh, potential_pv_kwp, num_intervals=35040
):
    ...
    return pd.DataFrame(
        {"consumption_kw": consumption_profile_kw, "production_kw": pv_profile_kw},
        index=timestamps,
    )
...
def calculate_community_autarky(community_buildings_df, all_profiles):
    ...
    for _, row in community_buildings_df.iterrows():
        ...
        all_profiles_sim[building_id] = generate_mock_profiles(
            row["annual_consumption_kwh"], row["potential_pv_kwp"]
        )
    ...
    for building_id in community_buildings_df["building_id"].values:
        ...
        community_consumption += pd.Series(
            profile["consumption_kw"].values, index=range(num_points)
        )
        community_production += pd.Series(
            profile["production_kw"].values, index=range(num_points)
        )
...
def get_cluster_info(community_df, cluster_id):
    ...
    autarky_score, total_consumption, total_production = calculate_community_autarky(
        community_df,
        None,  # Profile werden in der Fkt. neu generiert
    )
    ...
    for _, row in community_df.iterrows():
        members.append(
            {"building_id": row["building_id"], "lat": row["lat"], "lon": row["lon"]}
        )
...
def find_optimal_communities(building_data_df, radius_meters=150, min_community_size=3):
    ...
    for cluster_id in set(labels):
        ...
        community_buildings_df = pd.DataFrame(
            working_df[cluster_mask].to_dict("records")
        )
    ...
    if (
        "building_id" not in result_df.columns
        and "building_id" in building_data_df.columns
    ):
