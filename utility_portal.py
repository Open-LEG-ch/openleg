...
def register():
    ...
    db.track_event(
        "utility_registered",
        None,
        {"client_id": client_id, "company": company_name, "kanton": kanton},
    )
...
def login_submit():
    ...
    if not client:
        ...
        return jsonify(
            {
                "success": True,
                "message": "Falls registriert, erhalten Sie einen Login-Link per E-Mail.",
            }
        )
...
def generate_api_key():
    ...
    db.track_event(
        "utility_api_key_generated", None, {"client_id": client["client_id"]}
    )
