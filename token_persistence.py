...
TOKEN_FILE_FALLBACK = "./data/openleg_tokens.json"
...
def load_tokens():
    ...
    if not os.path.exists(file_path):
        ...
        print("[TOKEN DB] Starte mit leerer Token-Datenbank")
        return {}, {}, {}, {}
    ...
    try:
        ...
        if expired_verification or expired_unsubscribe:
            print(
                f"[TOKEN DB] {len(expired_verification)} abgelaufene Verification-Tokens entfernt"
            )
            print(
                f"[TOKEN DB] {len(expired_unsubscribe)} abgelaufene Unsubscribe-Tokens entfernt"
            )
...
def save_tokens(
    verification_tokens, unsubscribe_tokens, created_at=None, token_history=None
):
    """
    ...
    try:
        ...
        print(
            f"[TOKEN DB] Verification-Tokens: {len(verification_tokens)}, Unsubscribe-Tokens: {len(unsubscribe_tokens)}, Historie: {len(token_history)}"
        )
        ...
        print("[TOKEN DB] Tokens erfolgreich gespeichert")
        return True
    except Exception as e:
...
def save_tokens_async(
    ...
    def _save():
        with _persistence_lock:
            save_tokens(
                verification_tokens, unsubscribe_tokens, created_at, token_history
            )
